...
function ThreadNavigationSidebarPane(
  ...
  const deadEnvsSince = useRef<Map<EnvironmentId, number>>(new Map());
  ...
  useEffect(() => {
    ...
    for (const env of sandboxCleanupEnvs) {
      ...
      } else if (!failing.has(env.environmentId)) {
        prev.delete(env.environmentId);
      }
    }
  ...
  const hasSandboxEnvironment = useMemo(
    () =>
      Object.values(savedConnectionsById).some(
        (connection) => connection.displayUrl && isSandboxUrl(connection.displayUrl),
      ),
    [savedConnectionsById],
  );
