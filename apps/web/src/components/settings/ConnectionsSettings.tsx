...
function SavedBackendListRow({
  ...
  const metadataBits = [
    ...
    environment.relayManaged ? "Boundly Connect" : null,
  ].filter((value): value is string => value !== null);
...
function CloudLinkSwitch({
  ...
  disabledReason,
  onCheckedChange,
  ariaLabel = "Enable Boundly Connect",
}: {
...
function ConfiguredCloudLinkRow({ canManageRelay }: { readonly canManageRelay: boolean }) {
  ...
  const disabledReason = !isSignedIn
    ? "Sign in to Boundly Connect to manage this environment."
    : !canManageRelay
      ? "Your session does not have permission to manage Boundly Connect access."
      : null;
  ...
  const updateManagedTunnel = async (enabled: boolean) => {
    ...
    if (ok) {
      ...
      toastManager.add({
        type: "success",
        title: enabled
          ? "Boundly Connect linked"
          : publishAgentActivity
            ? "Boundly Connect tunnel disabled"
            : "Boundly Connect unlinked",
        description: enabled
          ? "This environment is available through Boundly Connect."
          : publishAgentActivity
            ? "The managed tunnel was removed. Agent activity publishing stays on."
            : "This environment is no longer available through Boundly Connect.",
      });
    }
    setIsUpdating(false);
  };
  ...
  return (
    <>
      <SettingsRow
        title="Boundly Connect"
        description={
          managedTunnelActive
            ? "This environment is available to your other devices through Boundly Connect."
            : "Make this environment available to your other devices through Boundly Connect."
        }
      ...
      <SettingsRow
        ...
        description="Send activity from this environment to your mobile clients for push notifications and Live Activities. Works without a Boundly Connect tunnel."
        control={
          <CloudLinkSwitch
  ...
  return (
    <Empty className="min-h-52">
      ...
      <EmptyHeader>
        ...
        <EmptyDescription>
          {cloudEnabled
            ? "Click “Add environment” to pair another environment, or connect one from Boundly Connect."
    ...
    return (
      <>
        ...
        {desktopWslState.enabled ? (
          <SettingsRow
            title="WSL only"
            description="Stop the Windows backend and run only the WSL backend. Useful if you develop entirely inside WSL and don't want a second backend process. Boundly restarts when you change this."
          ...
          >
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {pendingDesktopServerExposureMode === "network-accessible"
                    ? "Boundly will restart to expose this environment over the network."
                    : "Boundly will restart and limit this environment back to this machine."}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
            ...
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {pendingWslChange?.kind === "disable"
                    ? pendingWslChange.wasWslOnly
                      ? "Boundly will restart on the Windows backend. Threads and projects opened against WSL stay safe inside the distro and become available again when you re-enable WSL."
                      : "The WSL backend will stop. Threads and projects opened against WSL stay safe inside the distro, but they'll be unavailable in Boundly until you re-enable WSL."
                    : pendingWslChange?.kind === "distro"
                      ? "Boundly will restart the WSL backend on the new distro. Sessions still running on the current distro will be interrupted."
                      : pendingWslChange?.kind === "enable"
                        ? "Run the WSL backend alongside the Windows one, or stop the Windows backend and use only WSL? You can change this later from Settings."
                        : pendingWslChange?.nextValue
                          ? "Boundly will restart and start only the WSL backend. Your Windows-side projects won't be accessible until you turn this off again."
                          : "Boundly will restart and bring the Windows backend back up alongside WSL."}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
          ...
          >
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Boundly will restart the local backend without Tailscale Serve.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
          ...
          >
            <DialogPopup className="max-w-md">
              <DialogHeader>
                ...
                <DialogDescription>
                  Boundly will restart the local backend with Tailscale Serve enabled and ask
